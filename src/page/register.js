...
function createData(id, profile, name, email) {
  return { id, profile, name, email };
}
...
export default function StickyHeadTable() {
  ...
  const AddCredit = async () => {
    console.log(currentRow, "34567890-");
    let data = { _id: currentRow._id, credit: totalcredit };
    await axios
  ...
  const changestatus = async (row) => {
    ...
    await axios
      .post(`${process.env.REACT_APP_Base_Url}updatestatus`, { data: data })
      .then((result) => {
        if (result.data === "success") {
          getUserData();
          setOpen(false);
  ...
  const DeleteUser = (row) => {
    confirm({
      ...
      .then(() => {
        axios
          .post(`${process.env.REACT_APP_Base_Url}deleteUser`, { _id: row._id })
          .then((result) => {
            if (result.data === "success") {
              getUserData();
                    ...
                    {columns.map((column, k) => {
                      ...
                      return (
                        <TableCell key={k}>
                          ...
                          ) : column.id === "note" && value === "" ? (
                            "---"
                          ) : column.id === "_id" ? (
                            value.slice(0, 5) + "..." + value.slice(-5)
                          ) : column.id === "date" ? (
                            `${Math.floor(
                              (new Date().valueOf() - value) / 1000 / 60
                            )} mins ago`
                          ) : (
                            value
                          )}
          ...
          <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: "flex" }}>
              ...
              <Box sx={{ mt: 3, display: "flex", gap: 1 }}>
                ...
                <Button size="small" variant="contained" onClick={UpdateNote}>
                  Update
                </Button>
              </Box>
            </Box>
